# COMPACTION.md â€” Pre-Compaction Preservation Checklist
<!--
  INSTRUCTIONS:
  Execute this checklist BEFORE running /compact.
  Optimal compaction window: 30â€“38% context saturation.
  ...
  NEVER wait for auto-compaction (95%) â€” model is in degraded state at that threshold.
...
- [ ] Saturation is between 30â€“38% â†’ **proceed**
- [ ] Saturation is >40% â†’ **STOP â€” approaching sigmoid collapse cliff (43.2%). Execute State Freeze: FAILURE_LEDGER â†’ HANDOVER.md â†’ /clear**
- [ ] Pre-compact hook has run â†’ snapshot saved to `.claude/snapshots/`
...
<!--
  ...
  Be explicit â€” the model will omit anything not listed.
  Paste this list into the /compact command after "preserve:".
-->

### Compression Priority Order (execute in sequence)
Compress in this order â€” highest token yield first:
1. **Raw tool outputs** (bash stdout, grep results, file read dumps) â€” evict immediately
   after insight is absorbed; these are ~80% of context bloat
2. **Repeated exploratory reads** â€” collapse to conclusion only; discard the raw content
3. **Superseded reasoning traces** â€” paths that were evaluated and abandoned
...
### Active File Paths
- [ ] <!-- FILL: e.g. "src/auth/token-vault.ts â€” currently being refactored" -->
- [ ] <!-- FILL: e.g. "api/routes/checkout.ts â€” has uncommitted changes" -->

### Decisions (must survive compaction)
- [ ] <!-- FILL: e.g. "Decision to use PKCE over implicit flow â€” do not revisit" -->
- [ ] <!-- FILL: e.g. "Rejected passport.js â€” incompatible with session store" -->

### Active Blockers
- [ ] <!-- FILL: e.g. "TokenVault.rotate() has unresolved race condition â€” see line 142" -->

### Current Task State
- [ ] <!-- FILL: e.g. "Step 3 of 5 in auth refactor â€” middleware done, routes pending" -->
...
<!--
  ...
  A missing item = compaction failed to preserve it â†’ manually re-inject into next message.
-->
